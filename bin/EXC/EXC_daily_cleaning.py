...
daily_matrix['Pair'] = [element.replace(
    'USDT_BCHSV', 'bsvusdt') for element in daily_matrix['Pair']]
daily_matrix['Pair'] = [element.replace(
    'USDC_BCHSV', 'bsvusdc') for element in daily_matrix['Pair']]
daily_matrix['Pair'] = [element.replace(
    'USDT_BCHABC', 'bchusdt') for element in daily_matrix['Pair']]
daily_matrix['Pair'] = [element.replace(
...
day_before_matrix = mongo.query_mongo2(db, coll_clean, q_dict)
