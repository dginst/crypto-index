...
first_query = {'Pair': 'btcusd',
               'Exchange': 'kraken', 'Time': str(yesterday_TS)}
...
matrix_rate = mongo.query_mongo2(db, collection_rates, query_rate)
matrix_rate = matrix_rate.rename({'Date': 'Time'}, axis='columns')
matrix_data = mongo.query_mongo2(db, collection_data, query_data)
matrix_rate_stable = mongo.query_mongo2(db, collection_stable, query_stable)
...
if date_to_convert != []:
    ...
    for date in date_to_convert:
        ...
        matrix = mongo.query_mongo2(database, collection_converted, query_dict)
    ...
    print('Message: No new date to upload on on Mongo DB, the CW_final_data \
