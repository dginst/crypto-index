    ...
    for exchange in Exchanges:
        ...
        for cp in currencypair_array:
            ...
            matrix = daily_matrix.loc[
                (daily_matrix["Exchange"] == exchange) & (daily_matrix["Pair"] == cp)
            ]
...
daily_synt_rel = daily_synth_df.loc[Crypto_Asset].div(daily_synth_df["row_sum"], axis=0)


# find new divisor value
yest_price = np.array(two_before_price)
new_divisor_df = calc.new_divisor(
    yest_price, weights, logic_two, Crypto_Asset, old_reb_start, new_reb_start
)


# daily index value computation
curr_div_val = np.array(new_divisor_df["Divisor Value"])
index_numerator = np.array(Crypto_Asset_Prices[Crypto_Asset]) * np.array(daily_synt_rel)
