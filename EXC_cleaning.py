    ...
    for Crypto in Crypto_Asset:
        ...
        for exchange in Exchanges:
            ...
            for cp in currencypair_array:
                ...
                matrix = matrix.drop(columns = ['Ticker_time', 'Date','Bid', 'Ask', 'Traded_id'])
                ...
                if matrix.shape[0] > 1:
                    ...
                    matrix['Pair Volume'] = matrix['Crypto Volume'] * matrix['Close Price'] 
                ...
                # if the matrix is empty, the code searches the value in CW rawdata
                    ...
                    CW_matrix = mongo.query_mongo(database, collection_CW_raw, query_dict)
                    ...
                    if CW_matrix.shape[0] > 1:
                        ...
                        # selecting the date of interest
                        int_ref_vector = [int(date) for date in relative_reference_vector]
                        CW_matrix = CW_matrix.loc[CW_matrix['Time'].isin(int_ref_vector)]
                        CW_matrix['Pair Volume'] = CW_matrix['Crypto Volume'] * CW_matrix['Close Price']
                        # renaming CW_matrix
                        matrix = CW_matrix
                    ...
                    # if it not exists code takes the day before values
                        ...
                        yesterday_matrix = mongo.query_mongo(database, collection_clean_check, query_dict)
                        yesterday_date = [str(int(date) - 86400) for date in relative_reference_vector]
                        yesterday_matrix = yesterday_matrix.loc[yesterday_matrix['Time'].isin(yesterday_date )]
                        # renaming yesterday_matrix and changing "Time" column values
                        matrix = yesterday_matrix
                        matrix['Time'] = [str(int(date) + 86400) for date in matrix['Time']]             
