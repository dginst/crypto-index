...
parameters = {
    'startPeriod': '2019-12-01',  # Start date of the time series
...
# print(r)
for pair in ['btc']:
	cp = [""+pair+"usd", ""+pair+"eur", ""+pair+"cad", ""+pair+"gbp", ""+pair+"usdt", ""+pair+"usdc", ""+pair+"jpy"]

entrypoint = 'https://api.itbit.com/v1/'
resource = '/markets/ETHUSD/ticker'         



request_url = entrypoint + resource
response = requests.get(request_url, params=parameters, headers={'Accept': 'text/csv'})
print(response.url)
df = pd.read_csv(io.StringIO(response.text))
df.info()
ts = df.filter(['pair','volumeToday','lastPrice'], axis=1)
print(ts)
