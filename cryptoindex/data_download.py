...
def ECB_rates_extractor(key_curr_vector, Start_Period, End_Period=None, freq='D',
    ...
    for currency in key_curr_vector:
        key = freq + '.' + currency + '.' + curr_den + \
            '.' + type_rates + '.' + series_var
        ...
        date_timestamp = int(time.mktime(datetime.strptime(
            date_to_string, "%Y-%m-%d").timetuple())) + 3600
...
def CW_raw_to_mongo(exchange, currencypair, mongo_collection, start_date='01-01-2016', end_date=None, periods='86400'):
    ...
    key = exchange + "/" + currencypair + "/ohlc?periods=" + \
        periods + "&after=" + start_date + "&before=" + end_date
    ...
    try:
        ...
        if len(response['result']['86400']) == 0:
            ...
            rawdata = {'Exchange': Exchange, 'Pair': Pair, 'Time': Time, 'Low': Low, 'High': High, 'Open': Open,
                       'Close Price': Close_Price, 'Crypto Volume': Crypto_Volume, 'Pair Volume': Pair_Volume}
