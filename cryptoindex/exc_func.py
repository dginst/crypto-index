...
def log_key_update(key_to_update):
...
def exc_new_key_mng(logic_key_df, daily_mat_00, day_to_clean_TS):
    ...
    if new_key.empty is False:
        ...
        date_tot = [str(single_date) for single_date in date_tot_int]

        for key in new_key_list:

            # updating the logic matrix of exchange-pair couple
            logic_row_update = log_key_update(key)
            ...
            key_hist_df.loc[key_hist_df.Time
                            == day_to_clean_TS, "Close Price"] = new_price
            key_hist_df.loc[key_hist_df.Time
                            == day_to_clean_TS, "Pair Volume"] = new_p_vol
            key_hist_df.loc[key_hist_df.Time
                            == day_to_clean_TS, "Crypto Volume"] = new_c_vol
...
def exc_daily_cleaning(exc_list, date_tot_arr, day_to_clean=None):
    ...
    if day_to_clean is None:
        ...
        if date_to_clean != []:
            ...
            (daily_mat_00, daily_mat_12, daily_mat_16,
             daily_mat_20) = exc_time_split(daily_mat_vol_fix)
...
def exc_daily_operation(day=None):
    ...
    cleaned_df = exc_daily_cleaning(EXCHANGES, date_tot, day)
    mongo_upload(cleaned_df, "collection_exc_clean")
    converted_df = exc_daily_conversion(
        CONVERSION_FIAT, STABLE_COIN, date_tot, day)
