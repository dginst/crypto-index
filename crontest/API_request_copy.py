    ...
    for asset in Crypto:
        ...
        for fiat in Fiat:
            ...
            for start, stop in date_object:
                ...
                key = asset+"-"+fiat+"/candles?start="+start + \
                    "&end="+stop+"&granularity="+granularity
...
def kraken_API(Start_Date, End_Date, Crypto, Fiat, interval='1440'):
    ...
    header = ['Time', 'open', 'high', 'low',
              'Crypto Price', 'vwap', 'Crypto Volume', 'count']
    ...
    Kraken_df = Kraken_df.drop(
        columns=['open', 'high', 'low', 'vwap', 'count'])
...
def Poloniex_API(Start_Date, End_Date, Crypto, Fiat, period='86400'):
    ...
    df = np.array([])
    header = ['Time', 'open', 'high', 'low', 'Crypto Price',
              'Crypto Volume', 'volume_usd', 'vwap']
    ...
    for asset in Crypto:
        ...
        for fiat in Fiat:
            ...
            for start, stop in date_object:
                ...
                key = asset + '_' + fiat + '&start=' + \
                    start + '&end=' + stop + '&period=' + period
                request_url = entrypoint + key
